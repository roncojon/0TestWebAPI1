        ...
        public async Task<IActionResult> Put(Guid UId, [FromBody] ExamPostNeeds newExamData)
        {
            ...
            if (temp != null)
            {
                ...
                /* _dbContext.Examen.Remove(examenTemp);
                 await _dbContext.SaveChangesAsync();
                 _dbContext.Entry(newExamen).State = EntityState.Added;*/
                Examen9 existingExam = await _dbContext.Examen.FirstOrDefaultAsync((e) => e.UId == UId);
                existingExam.FechaFin = newExamen.FechaFin;
                existingExam.FechaInicio = newExamen.FechaInicio;
                existingExam.PatronClave = newExamen.PatronClave;
                existingExam.Test = newExamen.Test;
                existingExam.TestUId = newExamen.TestUId;

                //  await _dbContext.SaveChangesAsync();
                _dbContext.Entry(existingExam).State = EntityState.Modified;

                //CREANDO LA RELACION DE CADA USUARIO ASIGNADO AL EXAMEN
                List<UsuarioExamen10> existingUsersInExam = await _dbContext.UsuarioExamen.Where((u) => u.ExamenId == UId).ToListAsync();
                List<UsuarioExamen10> finalUsersInExam = new List<UsuarioExamen10>();
                foreach (var item in newExamData.UsuariosCiList)
                {
                    UsuarioExamen10 uExTemp = await _dbContext.UsuarioExamen.FirstOrDefaultAsync((u) => u.UsuarioCi == item && u.ExamenId == UId);
                    if (uExTemp == null)
                    {
                        ...
                        newUsuarioExamen.Fecha = 0;
                        finalUsersInExam.Add(newUsuarioExamen);
                ...
                foreach (var ue in existingUsersInExam)
                {
                    _dbContext.UsuarioExamen.Remove(ue);
                    await _dbContext.SaveChangesAsync();

                }
                if (finalUsersInExam.Count() > 0)
                {
